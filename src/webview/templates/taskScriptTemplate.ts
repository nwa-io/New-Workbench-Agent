  ...
  return `
    <script>
      ...
      const modeCopy = {
        ...
        analysis: {
          summary: 'Prepare context for a code or product analysis.',
      ...
      function updateModeControls() {
        ...
        if (currentView === 'detail' && currentItem) {
          ...
          summary.textContent = 'Create an item under ' + (taskState.projectFolder || '.project') + '.';
        } else {
          summary.textContent = (modeCopy[currentMode] || modeCopy.task).summary;
        }
      }
      ...
      function renderListView() {
        ...
        title.textContent = itemTypePluralLabel(activeType);
        meta.textContent = visibleItems.length + ' ' + (visibleItems.length === 1 ? itemTypeText(activeType) : itemTypePluralLabel(activeType).toLowerCase()) + ' in ' + (taskState.projectFolder || '.project') + '/' + activeType;
        ...
        list.innerHTML = visibleItems.length > 0
          ...
          : '<p class="empty-state">No ' + itemTypePluralLabel(activeType).toLowerCase() + ' created yet.</p>';
      ...
      function validateCreateStep(step) {
        ...
        if (step === 3 && !isCreateWorkflowValid()) {
          createFormState.message = getWorkflowLayouts().length === 0
            ? 'Create a workflow layout in Workflow Settings first.'
            : 'Select a workflow layout.';
          createFormState.isError = true;
          return false;
        }
      ...
      function renderWorkflowLayoutPreview() {
        ...
        const workflows = getWorkflowLayouts();
        if (workflows.length === 0) {
          preview.innerHTML = '<div class="workflow-layout-empty">No workflow layout YAML files found in .project/workflows.</div>';
          return;
        }

        const workflow = selectedCreateWorkflow();
        if (!workflow) {
          preview.innerHTML = '<div class="workflow-layout-empty">Select a workflow layout.</div>';
          return;
        }
        ...
        if (blocks.length === 0) {
          preview.innerHTML = getReadonlyWorkflowShell(
            workflow,
            fileName,
            '<div class="tree-empty">This workflow layout has no steps.</div>'
          );
          return;
        }
      ...
      function renderTree() {
        ...
        const flowNodes = [
          createFlowNode('document', 44, 72, 238, 96, 'wide', modeCopy[currentMode]?.documentTitle || 'Document', documentCount > 0 ? documentCount + ' markdown file' + (documentCount === 1 ? '' : 's') : 'Drop SRS, PDF, notes'),
      ...
      function updateFigmaNodeSelection(nodeId, isSelected) {
        ...
        figmaFormState.message = selectedNodeIds.length + ' Figma node' + (selectedNodeIds.length === 1 ? '' : 's') + ' selected for this ' + itemTypeText(modeToItemType(currentMode)) + '.';
        figmaFormState.isError = false;
      ...
      function renderDocumentDetail(detail) {
        const copy = modeCopy[currentMode] || modeCopy.task;

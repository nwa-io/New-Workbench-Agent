/**
 * Client-side detail UI for the Collect Jira step, concatenated into the Task
 * Manager webview's single global scope by core's webview assembly. It reads the
 * shell-owned globals `taskState`, `jiraFormState`, `escapeHtml`, `statusClass`,
 * and `formatDateTime`, and defines `renderJiraDetail` / `captureJiraFields`
 * which the shell's detail view invokes by name.
 */
export const jiraDetailScript = `

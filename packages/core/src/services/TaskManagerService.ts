...
import { JiraScraper } from '@nwa/step-collect-jira';
import { FigmaClient, ParsedFigmaLink } from '@nwa/step-collect-figma';
import { buildDocumentJudgmentPrompt, parseDocumentJudgmentReport } from '@nwa/step-collect-document';
import { buildSpecMarkdown } from '@nwa/spec-generator';
...
export class TaskManagerService {
  ...
  private jiraScraper = new JiraScraper();
  ...
  async syncFigmaLink(
    ...
    const figmaLink = this.figmaClient.parseLink(request.link);
    const response = await this.figmaClient.fetchFile(figmaLink, token);
    const figmaNodes = this.figmaClient.getNodes(response);
    this.figmaClient.ensureNodeExists(response, figmaLink.nodeId);
    ...
    const connection: TaskFigmaConnection = {
      ...
      nodeName: this.figmaClient.getNodeName(response, figmaLink.nodeId),
  ...
  async updateFigmaNodeSelection(
    ...
    const selectedNodeIds = this.figmaClient.getUniqueNodeIds(request.selectedNodeIds)
  ...
  async setWorkflowStepModel(
    request: TaskWorkflowRunRequest & { stepId?: string; model?: string; speed?: string }
  ): Promise<{ workflow: WorkflowFile; state: TaskManagerState }> {
    return this.updateWorkflowStepById(request, request.stepId, step => {
      step.model = request.model;
      // Speed accompanies a model change (Codex only); undefined clears it for Claude.
      step.modelSpeed = request.speed;
    });
  }

  async setWorkflowStepSpeed(
    request: TaskWorkflowRunRequest & { stepId?: string; speed?: string }
  ): Promise<{ workflow: WorkflowFile; state: TaskManagerState }> {
    return this.updateWorkflowStepById(request, request.stepId, step => {
      step.modelSpeed = request.speed;
    });
  ...
  private async updateWorkflowStepById(
    request: TaskWorkflowRunRequest,
    stepId: string | undefined,
    mutate: (step: WorkflowStepBlock) => void
  ): Promise<{ workflow: WorkflowFile; state: TaskManagerState }> {
    const workspaceFolder = await this.requireWorkspaceFolder();
    const item = await this.resolveOperationItem(workspaceFolder, request.mode || 'task', request);
    const workflow = await this.resolveTaskWorkflow(workspaceFolder, item);
    const matched = stepId ? this.findWorkflowStepForCompletion(workflow, { stepId }) : undefined;

    if (!matched) {
      throw new Error('The selected workflow step was not found.');
    }
    if (matched.step.status !== 'idle') {
      throw new Error('This step has already run; its model can no longer be changed.');
    }
  ...
  async judgeTaskDocumentsWithClaude(
    ...
    try {
      ...
      let judgment = parseDocumentJudgmentReport(report);
      ...
      if (!judgment.statusFound) {
        ...
        judgment = parseDocumentJudgmentReport(normalizedReport);
      }
  ...
  async openJiraInChrome(
    ...
    const workspaceFolder = await this.requireWorkspaceFolder();
    const item = await this.resolveOperationItem(workspaceFolder, request.mode || 'task', request);
    const jiraLink = this.jiraScraper.parseLink(request.link);
    const profilePath = await this.getJiraProfilePath();
    await this.jiraScraper.open(profilePath, jiraLink);
  ...
  async readJiraTicket(
    ...
    const workspaceFolder = await this.requireWorkspaceFolder();
    const item = await this.resolveOperationItem(workspaceFolder, request.mode || 'task', request);
    const jiraLink = this.jiraScraper.parseLink(request.link || this.jiraConnection?.link || '');
    const profilePath = await this.getJiraProfilePath();

    const ticket = await this.jiraScraper.read(profilePath, jiraLink);
  ...
  private async generateTaskMarkdown(mode: TaskManagerMode, item: TaskManagerItem): Promise<string> {
    ...
    let content = buildSpecMarkdown(guide, {
      mode,
      item,
      documentSummaries,
      documentsFolder,
  ...
  private async createDocumentJudgmentPrompt(
    ...
    const judgmentDocuments = await Promise.all(documents.map(async document => {
      ...
      return { name: document.name, workspacePath: document.workspacePath, content };
    }));

    return buildDocumentJudgmentPrompt(judgmentDocuments, documentsFolder, guide);
  }
  ...
  private getInitialFigmaSelectedNodeIds(nodes: TaskFigmaNode[], figmaLink: ParsedFigmaLink): string[] {
    ...
    return this.figmaClient.getUniqueNodeIds([
  ...
  private createFallbackTaskWorkflow(item: TaskManagerItem): WorkflowFile {
    return {
      ...
      blocks: [
        ...
        this.createFallbackStep('code', 'code', 'Code'),

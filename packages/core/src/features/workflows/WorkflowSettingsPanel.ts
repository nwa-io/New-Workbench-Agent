...
import { openExternalPowerShellCommand } from '../../utils/externalTerminal';
...
import { getCoreSavePathSettings } from './settingsData';
import { renderWebviewHtml, webviewLocalResourceRoots } from '../../webview/webviewHtml';
...
type SettingsTab = 'core' | 'workflows';
...
export class WorkflowSettingsPanel {
  ...
  private activeTab: SettingsTab = 'core';
  /** Models offered in the builder, gated by which CLIs are authenticated. */
  ...
  public static createOrShow(
    ...
    if (WorkflowSettingsPanel.currentPanel) {
      ...
      WorkflowSettingsPanel.currentPanel.setActiveTab(initialTab);
      void WorkflowSettingsPanel.currentPanel.postCoreSettings();
      return;
    }
    const panel = vscode.window.createWebviewPanel(
      'NWAWorkflowSettings',
      WorkflowSettingsPanel.titleForTab(initialTab),
    ...
    WorkflowSettingsPanel.currentPanel = new WorkflowSettingsPanel(panel, initialTab, extensionUri, configService);
  }

  private static titleForTab(tab: SettingsTab): string {
    return tab === 'workflows' ? 'NWA Workflows' : 'NWA Settings';
  }
  ...
  private constructor(
    ...
    this.panel.webview.html = renderWebviewHtml({
      ...
      title: WorkflowSettingsPanel.titleForTab(initialTab)
    });
  ...
  private setActiveTab(tab: SettingsTab): void {
    this.activeTab = tab;
    this.panel.title = WorkflowSettingsPanel.titleForTab(tab);
    this.panel.webview.postMessage({ command: 'setActiveTab', data: { tab } });
  }
    ...
    try {
      switch (msg.command) {
        ...
        case 'deleteBlock':
          ...
          break;
        case 'setStepModel':
          await this.setStepModel(msg.data?.workflowId, msg.data?.locator, msg.data?.model);
          break;
        case 'setStepSpeed':
          await this.setStepSpeed(msg.data?.workflowId, msg.data?.locator, msg.data?.speed);
          break;
  ...
  private async getCoreSettingsState(): Promise<{
    ...
    const cliStatuses = await this.getCliStatuses();
    const authedProviders = cliStatuses
      .filter(status => status.authenticated)
      .map(status => status.id as ModelProvider);
    this.availableModels = await this.modelCatalog.buildAvailableModels(authedProviders);
    return {
  ...
  private async authenticateCli(id: CliToolId | undefined): Promise<void> {
    ...
    if (process.platform === 'win32') {
      await openExternalPowerShellCommand({
        command,
        cwd: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath,
        title: `NWA ${label} Auth`,
        completionMessage: `${label} authentication command finished. Return to NWA Settings and refresh after sign-in completes.`
      });
      await this.postCoreSettingsSaved('cli-status-status', `${label} authentication opened in an external PowerShell window. Refresh after it completes.`);
      return;
    }
  ...
  private postState(): void {
    this.panel.webview.postMessage({
      command: 'setState',
      data: { workflows: this.workflows, activeId: this.activeId, availableModels: this.availableModels }
    });
  }
  ...
  private async pickStep(): Promise<WorkflowStepBlock | undefined> {
    ...
    return {
      ...
      title: picked.label,
      status: 'idle',
      model: defaultModelForStep(picked.stepType)
    };
  }
  ...
  private async setStepModel(
    ...
    const step = this.resolveStep(wf, locator);
    if (!step) {
      return;
    }
    step.model = model;
    // Reset the speed to the new model's default (one below max); clear it for
    // models with no speed axis (Claude).
    const option = this.findModelOption(model);
    step.modelSpeed = option?.speeds && option.speeds.length > 0 ? option.defaultSpeed : undefined;
    await this.storage.saveWorkflow(wf);
    this.postState();
  }

  private async setStepSpeed(
    workflowId: string,
    locator: Locator,
    ...
    step.modelSpeed = speed;
    await this.storage.saveWorkflow(wf);
    this.postState();
  }

  private findModelOption(modelId: string): ModelOption | undefined {
    return (
      this.availableModels.find(model => model.id === modelId) ??
      MODEL_CATALOG.find(model => model.id === modelId)
    );
  }

  private resolveStep(wf: WorkflowFile, locator: Locator): WorkflowStepBlock | undefined {
    if (locator.type === 'root') {
      const block = wf.blocks[locator.index];
      return block && block.kind === 'step' ? block : undefined;
    }
    const parent = wf.blocks[locator.parentIndex];
    if (parent && parent.kind === 'parallel') {
      return parent.children[locator.childIndex];
    }
    return undefined;
  }
